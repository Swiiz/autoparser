...
pub mod ast {
    ...
    autoparser::impl_rules! {
      ...
      enum Expr => GroupExpr | Term,
      GroupExpr { expr: Box<Expr> } => (OpenParen {}, expr, CloseParen {}),
      ...
      MulOperation { left: Unary, right: Box<Factor> } => (left, MulOperator {}, right),
      DivOperation { left: Unary, right: Box<Factor> } => (left, DivOperator {}, right),

      enum Unary => InverseOperation | Primary,
      InverseOperation { primary: Primary } => (SubOperator {}, primary),

      enum Primary => Literal | Identifier | GroupExpr,
    }
}
